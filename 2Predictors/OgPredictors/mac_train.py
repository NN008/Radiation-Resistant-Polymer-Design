...
# 1. Load dataset
# -------------------------
df = pd.read_csv('/csl/users/2026nnandaku/cluster/PolymerDesign/1Dataset/PI1M_Tg_MAC.csv')
df = df[df['MAC'].notna()].copy()
...
# -------------------------
drop_cols = ['SMILES', 'Name', 'MAC', 'Tg', 'Tg_LF', 'Tg_predicted']
X_sample = df_sample.drop(columns=[col for col in drop_cols if col in df_sample.columns])
y_sample = df_sample['MAC']
...
# -------------------------
X_full = df.drop(columns=[col for col in drop_cols if col in df.columns])
y_full = df['MAC']
...
print(f"\nFinal Model MAE:  {mae:.6f}")
print(f"Final Model RMSE: {rmse:.6f}")
print(f"Final Model R²:   {r2:.6f}")
sys.stdout.flush()

# -------------------------
# 7. Save final model
# -------------------------
model_path = '/csl/users/2026nnandaku/cluster/PolymerDesign/mac_predictor_rf_final7.pkl'
